...
3. Batch processing small files saves time and money on spark on AWS Lambda in comparison with AWS EMR( with minimmum of 3 nodes). The framework can be hosted on AWS Batch for longer-running batch processes or AWS Lambda for 15-minute loads.
...
```
docker tag  sparkonlambda:latest 123456789012.dkr.ecr.us-east-1.amazonaws.com/sparkonlambda:latest</code> 
```
###  AWS ECR push to repository

```
docker push <accountnumber>.dkr.ecr.us-east-1.amazonaws.com/sparkonlambda</code>
```
...
Fill in the function name and the environment variable information. The environment variable can be the configuration input and output file locations or the configuration JSON file.
e.g. configFileLocation

Select the memory and timeout requirements, then configure the hardware, VPC, task role IAM, and enable logging in AWS Cloudwatch.Then click next.

Use a sample event to trigger the AWS lambda and watch the Cloudwatch logs
